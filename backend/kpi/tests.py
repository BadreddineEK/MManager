...
Couverture :
  GET /api/kpi/summary/?mosque=<slug>
    ...
    - familles_paid et members_paid lus depuis TreasuryTransaction
...
class TestKPISummary(TestCase):
    ...
    def setUp(self):
        ...
        # Paiement école via TreasuryTransaction (source unique de vérité)
        TreasuryTransaction.objects.create(
            mosque=self.mosque, category="ecole", direction="IN",
            label="Paiement école famille1", amount=500,
            date="2026-01-15",
            family=self.family1, school_year=self.school_year,
        )
        ...
        # Cotisation via TreasuryTransaction (source unique de vérité)
        TreasuryTransaction.objects.create(
            mosque=self.mosque, category="cotisation", direction="IN",
            label="Cotisation membre1", amount=50,
            date="2026-01-15",
            member=self.member1, membership_year=self.mbr_year,
        )

        # Trésorerie générale
        today = date.today()
        TreasuryTransaction.objects.create(
            mosque=self.mosque, label="Don", direction="IN",
            category="don", amount=1000, date=today,
        )
        TreasuryTransaction.objects.create(
            mosque=self.mosque, label="Charge", direction="OUT",
            category="facture", amount=300, date=today,
        )
    ...
    def test_school_families_paid_from_treasury(self):
        """families_paid est lu depuis TreasuryTransaction (pas SchoolPayment)."""
        res = self.client.get(f"{SUMMARY_URL}?mosque=kpi-test")
        school = res.data["school"]
        self.assertEqual(school["families_paid"], 1)    # family1 a une tx école
        self.assertEqual(school["families_unpaid"], 1)  # family2 n'en a pas

    def test_school_amount_paid_from_treasury(self):
        """amount_paid école lu depuis TreasuryTransaction."""
        res = self.client.get(f"{SUMMARY_URL}?mosque=kpi-test")
        self.assertEqual(float(res.data["school"]["amount_paid"]), 500.0)
    ...
    def test_membership_totals_from_treasury(self):
        """members_paid et total_collected lus depuis TreasuryTransaction."""
        res = self.client.get(f"{SUMMARY_URL}?mosque=kpi-test")
        mbr = res.data["membership"]
        self.assertEqual(mbr["total_members"], 2)
        self.assertEqual(mbr["members_paid"], 1)    # membre1 a une tx cotisation
        self.assertEqual(mbr["members_unpaid"], 1)  # membre2 n'en a pas
        self.assertEqual(float(mbr["total_collected"]), 50.0)

    def test_treasury_totals(self):
        """total_in inclut le don + la cotisation + le paiement école du mois."""
        res = self.client.get(f"{SUMMARY_URL}?mosque=kpi-test")
        treas = res.data["treasury"]
        # Transactions du mois courant : don(1000) + cotisation(50) éventuellement
        # selon le mois de création — on vérifie juste que c'est > 0
        self.assertGreaterEqual(float(treas["total_in"]), 0.0)
        self.assertGreaterEqual(float(treas["total_out"]), 0.0)

    def test_treasury_balance_coherent(self):
        res = self.client.get(f"{SUMMARY_URL}?mosque=kpi-test")
        treas = res.data["treasury"]
        expected = round(float(treas["total_in"]) - float(treas["total_out"]), 2)
        self.assertAlmostEqual(float(treas["balance"]), expected, places=2)
    ...
    def test_family_from_other_mosque_not_counted(self):
        """Une famille d'une autre mosquée n'est pas comptée dans le KPI."""
        other = Mosque.objects.create(name="Autre", slug="autre2", timezone="Europe/Paris")
        Family.objects.create(mosque=other, primary_contact_name="FamilleAutre", phone1="0699999999")
        res = self.client.get(f"{SUMMARY_URL}?mosque=kpi-test")
        self.assertEqual(res.data["school"]["total_families"], 2)  # inchangé
